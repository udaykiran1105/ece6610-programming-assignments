...
int xilnet_ip(unsigned char*buf, int len, unsigned int eth_dev_num) {
    ...
    if (addr_pass == 1) {
        ...
        // WARP hardware does not support fragmented packets.  However, we will still pass
        // the first fragment of a packet up to the transport for processing so that the 
        // OS fragmenting packets does not result in a transport timeout in initNodes.  If
        // this behavior needs to change the below code will cause the transport to discard
        // packet fragments.
        //         
        ...
        // 
        // if ((iph->frag_off & 0xFFBF) != IP_FRAG_OFF) {
        //     xil_printf("ERROR:  WARPxilnet does not support fragmented packets.\n");
        //     return 0;
        // }
    ...
    // call corr protocol handler with the ip datagram
        switch (iph->prot) {
	        ...
	        break;
            case IP_PROTO_TCP:
                xil_printf("ERROR:  WARPxilnet does not support TCP\n");
            	return 0;
	        break;
            case IP_PROTO_ICMP:
